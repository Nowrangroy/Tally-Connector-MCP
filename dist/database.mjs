...
export async function cacheTable(lstColumnMetadata, data) {
    try {
        // Always create a table, even when Tally returns zero rows.
        // Returning an empty tableID caused downstream MCP tools / Claude to crash
        // after table-producing tools such as ledger-account returned { tableID: "" }.
        if (!Array.isArray(data))
            data = [];
        if (!(lstColumnMetadata instanceof Map) || lstColumnMetadata.size === 0)
            throw new Error('cacheTable requires non-empty column metadata');
